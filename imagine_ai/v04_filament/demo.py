#!/usr/bin/env python3
"""
demo.py - imagineAI v0.4 Demo

Semantic Filaments + Gradient Flow + DTW

Sentences are PATHS through semantic space.
The GRADIENTS between words encode meaning flow.
σ = DTW distance between gradient sequences.

NO NEURAL NETWORK in the solver. Pure math.
"""
...
def main():
    print("=" * 70)
    print("imagineAI v0.4 - Semantic Filaments")
    print("Sentences as PATHS. Gradients as MEANING FLOW.")
    print("=" * 70)
    
    from imagine_ai.v04_filament.solver import create_demo_solver
    
    print("\n[1] Initializing...")
    solver = create_demo_solver(verbose=False)
    
    print("\n[2] Ready! Ask questions (type 'quit' to exit)")
    print("    Type 'trace' to see detailed analysis.\n")
        ...
        try:
            ...
            if question.lower() == 'trace':
                ...
                if trace_q:
                    ...
                    print(f"Words: {trace['question_words']}")
                    print(f"Gradients: {trace['question_gradients']}")
                    print(f"Curvature: {trace['question_curvature']:.4f}")
                    print(f"\nTop results by σ:")
                    for r in trace['results'][:5]:
                        print(f"  [{r['rank']}] σ={r['sigma']:.4f}: {r['text'][:45]}...")
                    print("--- END TRACE ---\n")
                continue
...
def run_tests():
    """Run test suite."""
    print("=" * 70)
    print("imagineAI v0.4 - Filament Test Suite")
    print("=" * 70)
